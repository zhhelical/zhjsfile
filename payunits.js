...
var firstSignFunc = function(s_id, order, h_goods, cny, cb){
    sign_random(function(res){
        if(res != 'err for sign random'){
            var stringA = `appid=${appid}&body=h_goods&mch_id=${mch_id}&nonce_str=${res}&notify_url=${notify_url}&out_trade_no=${order}&openid=${s_id}&spbill_create_ip=${h_ip}&total_fee=${cny}&trade_type=JSAPI`
...
var secondSignFunc = function(pay_id, seconds, cb){
    sign_random(function(res){
        if(res != 'err for sign random'){
            var stringB = `appid=${appid}&timeStamp=${seconds}&nonce_str=${res}&package=${pay_id}&signType=MD5`
            var sign = crypto.createHash('md5').update(`${stringB}`, 'utf-8').digest('hex').toUpperCase()
...
module.exports = {
    ...
    genSecondsSign:function(p_id, seconds, cb){
        secondSignFunc(pay_id, seconds, function(s_res){
