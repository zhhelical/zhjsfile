...
var signFunc = function(order, h_goods, cny, cb){
    sign_random(function(res){
        if(res != 'err for sign random'){
            //var js_body = JSON.stringify(body)
            var stringA = `appid=${appid}&body=h_goods&mch_id=${mch_id}&nonce_str=${res}&notify_url=${notify_url}&out_trade_no=${order}&spbill_create_ip=${h_ip}&total_fee=${cny}&trade_type=JSAPI`
...
module.exports = {
    ...
    options: function(p_id, order, h_goods, cny, cb){
        signFunc(order, h_goods, cny, function(r_sign){
