    ...
    conn.on('message', function(message) {
        ...
        if(mData.key == 'sessionKey'){
            ...
            mysql.select_picpos(mData.value.time, mData.value.key).then(function(res_poses){
                ...
                var pos = res_poses[0].img_pos
                conn.send(JSON.stringify({key: 'getImagePos', value: {img_pos:pos, time:mData.value.time}}))
            }).then(function(res_err){
                if(res_err != undefined)
                    conn.send(JSON.stringify({key: 'getImagePos', value: 'failed getImagePos'}))
            })
        }
        else if(mData.key == 'getImageName'){
            mysql.select_picname(mData.value.time, mData.value.key, mData.value.pos).then(function(res_poses){
                console.log(res_poses)
                var name = res_poses[0].img_name
                conn.send(JSON.stringify({key: 'getImageName', value: {img_name:name, time:mData.value.time}}))
            }).then(function(res_err){
                if(res_err != undefined)
                    conn.send(JSON.stringify({key: 'getImageName', value: 'failed getImageName'}))
            })
        }
