...
var mark = function(req, res) {
    console.log('mark listener removing')
    res.end()
}
wss_server.on('connection', function(conn){
                    ...
                    request(options, function(error, response, body){
                        if (!error && response.statusCode == 200) {
                            ...
                            if(info.openid == 'undefined'){
                                feedback = {key: mData.key, value: 'openid undefined'}
                                conn.send(JSON.stringify(feedback))
                                return
                            }
    ...
    conn.removeListener('connection', mark)
})
wss_server.on('removeListener', function(e, f) {
    console.log('removeListener')
})
//wss_server.removeListener('connection', mark)
