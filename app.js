...
wss_server.on('connection', function(conn){
    conn.on('message', function(message) {
        ...
        if(mData.key == 'sessionKey'){
            ...
            joiners.updateConn(mData.value.localKey, conn)
            var chk_id = joiners.findByKey(mData.value.localKey)
            if(chk_id.openid == master.id_master){
                master.getSuitableSubmits(mData.value.from).then(function(g_res){
                    conn.send(JSON.stringify({key: mData.key, value: g_res}))
                }).then(function(err){
                    if(err)
                        conn.send(JSON.stringify({key: mData.key, value: err}))
                    else
                        conn.send(JSON.stringify({key: mData.key, value: 'no submits now'}))
                })
            }
            else {
            ...
            if(openid == master.id_master){
                master.getSuitableMsgs(mData.value.length.head).then(function(g_res){
                    conn.send(JSON.stringify({key: mData.key, value: g_res}))
                }).then(function(err){
                    if(err)
                        conn.send(JSON.stringify({key: mData.key, value: err}))
