        ...
        if(mData.key == 'sessionKey'){
            ...
            redis.getCacheValue(key).then(function (val) {
                if(val) {
                    var cache_objs = JSON.parse(val)
                    var old_caches = cache_objs.slice(0)
                    var delarray = function(dels){
                        if(!dels.length) {
                            conn.send(JSON.stringify({key: 'delUnpayeds', value: 'delUnpayeds finished'}))
                            return
                        }
                        var d_time = dels[0]
                        for(var ci in old_caches){
                            if(old_caches[ci].time == d_time) {
                                old_caches.splice(ci, 1)
                                break
                            }
                        }
                        redis.storeCache(key, JSON.stringify(old_caches)).then(function (res) {
                            dels.splice(0, 1)
                            delarray(dels)
                        }).then(function (err) {
                            if (err) {
                ...
                if(err){
                    var js_key = JSON.parse(key)
                    joiners.appOptErr(js_key.openid, null, `${err}`, 'app.delUnpayeds.redis.getCacheValue', 'people', mData.value, 'null')
                }
            })
        }
