...
var reqGetImagesProps = function(mData, conn){
    ...
    mysql.select_picsprops(mData.value.time, mData.value.key).then(function(res_rows){
        sendEndDeal(mData.key, conn, {imgs_rows:res_rows, time:mData.value.time})
        ...
        if(res_err) {
            ...
            sendEndDeal(mData.key, conn, 'failed getImagesProps')
        }
    })
}
var reqPayment = function(mData, conn){
    find_openid(mData.value.localKey, conn, function(f_res){
        if(f_res){
            ...
            recurseForWxHide(mData, conn, null, reqPayment)
    })
}
var reqSearchPeople = function(mData, conn){
    find_openid(mData.value.localKey, conn, function(f_res){
        if(f_res){
            var chk_id = f_res.openid
            ...
            recurseForWxHide(mData, conn, 'searchPeople failed', reqSearchPeople)
    })
}
...
var reqMsgsSubmit = function(mData, conn){
    find_openid(mData.value.localKey, conn, function(f_res){
        if(f_res){
            ...
            recurseForWxHide(mData, conn, 'failed', reqMsgsSubmit)
    })
}
var reqGetMsgs = function(mData, conn){
    find_openid(mData.value.localKey, conn, function(f_res){
        if(f_res){
            ...
            recurseForWxHide(mData, conn, 'failed', reqGetMsgs)
    })
}
