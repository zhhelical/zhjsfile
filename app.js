            ...
            request(options, function(error, response, body){
                if (!error && response.statusCode == 200) {
                    ...
                    if(!mysql.private_sql)
                        mysql.connectDb()
                    mysql.selectUser(`*${info.openid}*`, function(data) {
                        if (data) {
                            ...
                            session_random.shellFunc.then(function (value) {
                                ...
                                if (get_older)
                                    redis.storeCache(redis_keys, get_older.data)
                                else
                                    redis.storeCache(redis_keys, 'test')
            ...
            redis.getCacheKey(`*${mData.content.localKey}*`, function(res){
                ...
                redis.getCacheValue(srv_keys, function(res){
                    if(res){
                        ...
                        redis.storeCache(srv_keys, mData.content)
                        var ins_obj = {key:srv_keys, value:mData.content}
                        mysql.insertText(ins_obj, function(res){
                            if(res) throw res
