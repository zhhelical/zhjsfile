...
wss_server.on('connection', function(conn){
    conn.on('message', function(message) {
                            ...
                            request(res_opt.value.urlobj, function(error, response, body){
                                if (!error && response.statusCode == 200) {
                                    ...
                                    xmlparser.parseString(body, function (err, result) {
                                        console.log(JSON.stringify(result.xml.prepay_id))
                                        ...
                                        payprocess.genSecondsSign(payid, currentsecs, function(getsign){
                                            if(getsign) {
                                                const pay_notify = require('../../nodesrv/respondnotify.js')//change to clients after spring festival
                                                var payObjects = {openid: openid, time: mData.value.stime, out_trade_no: res_opt.value.out_trade_no, total_fee:res_opt.value.total_fee}
                                                pay_notify.responseObj.push(payObjects)
        ...
        else if(mData.key == 'getNotifySign') {
            conn.send(JSON.stringify({key: 'getNotifySign', value: joiners.getFailedSign()}))
        }
