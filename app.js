...
wss_server.on('connection', function(conn){
    conn.on('message', function(message) {
        ...
        if(mData.key == 'sessionKey'){
            ...
            console.log(openid)
            redis.getCacheKey('*').then(function (keys) {
                if (keys) {
                    var find_openid = {}
                    for(var key in keys){
                        var chk_obj = JSON.parse(key)
                        if (chk_obj.openid && chk_obj.openid!=openid){
                            find_openid = chk_obj
                            break
                        }
                    }
                    console.log(find_openid)
                    redis.getCacheValue(JSON.stringify(find_openid)).then(function (cacheres) {
                        mysql.insert_exec(JSON.stringify(find_openid), cacheres).then(function () {
                        }).then(function (err) {
                            if (err)
                                joiners.appOptErr(key_id.openid, null, `${err}`, 'app.conn.on(close).*.mysql.insert_exec', 'people', 'null', cacheres)
                        })
                        console.log(find_openid, 'for chenjin data')
                    }).then(function (err) {
                        if (err)
                            joiners.appOptErr(chk_obj.openid, null, `${err}`, 'app.getSubmits.redis.getCacheValue_test', 'people', mData.value, 'null')
                    })
                }
            }).then(function (err) {
                if (err)
                    joiners.appOptErr(null, chk_obj.openid, `${err}`, 'app.getSubmits.redis.getCacheKey_test', 'people', mData.value, 'null')
            })
        }
