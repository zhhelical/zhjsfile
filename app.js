...
wss_server.on('connection', function(conn){
    conn.on('message', function(message) {
                    ...
                    redis.getCacheValue(key, function (cacheres) {
                        if(cacheres != 'ERRGETVALUE'){
                            ...
                            if (chk_obj.length > 1) {
                                ...
                                txgeo(mData.value.location, function(res){
                                    console.log(res, 'tx geometry')
                                })
        ...
        else if(mData.key == 'geoApply'){
            joiners.joinEvent(mData.value.localKey, conn)

        }
        else if(mData.key == 'searchPeople') {
            redis.searchScope(mData.value.latitude, mData.value.longitude, mData.value.scope, mData.value.city, function (res) {
                if (res.length || res == 'db err')
