...
wss_server.on('connection', function(conn){
    conn.on('message', function(message) {
        ...
        if(mData.key == 'sessionKey'){
            ...
            request(options, function(error, response, body){
                if (!error && response.statusCode == 200) {
                    ...
                    mysql.newComerTest(`${info.openid}`, function(data) {
                        if (data.length) {
                            var feedback = {key: mData.key, value: data.localKey}
            ...
            var submit_str = JSON.stringify(mData.value.nest)
            redis.getCacheKey(`*${mData.value.localKey}*`, function(key){
                var srv_keys = JSON.stringify(key)
                redis.getCacheValue(srv_keys, function(val){
                    if(val){
                        var cache_objs = val
                        ...
                        redis.storeCache(srv_keys, JSON.stringify(cache_objs))
                        mysql.text_exec(srv_keys).then(function(result){//这里要变成用户下线时批量插入
                            if(result.length){
                                var merge = redis.cacheSqlComp(val, result)
                                mysql.update_exec(srv_keys, merge).then(function(res){
                                ...
                                mysql.insert_exec(srv_keys, JSON.stringify(cache_objs)).then(function(res){
                                    conn.send(JSON.stringify({key:'submitInfo', value:'submit received'}))
                                }).then(function(err){
                                    conn.send(JSON.stringify({key:'submitInfo', value:'submit failed'}))
                                })
