...
var reqMasterGets = function(mData, conn, tbl){
    mysql.masterGetting(tbl).then(function (s_res) {
        if(s_res.length)
            sendEndDeal(mData.key, conn, s_res)
    }).then(function (err) {
        if (err) {throw err}
...
var reqRestart = function(conn){
    ...
    joiners.appOptErr(null, null, `${err}`, 'app.conn.on(restart)', 'null', 'null', 'pm2 restart wscsrv')
}
...
wss_server.on('connection', function(socket){
    ...
    socket.on('message', function(message) {
        ...
        if(mData.key == 'sessionKey')
            ...
            reqMasterGets(mData, socket, mData.value)
