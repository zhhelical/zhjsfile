...
wss_server.on('connection', function(conn){
    conn.on('message', function(message) {
        ...
        if(mData.key == 'sessionKey'){
                ...
                if (!error && response.statusCode == 200) {
                    ...
                    mysql.newComerTest(`${info.openid}`, function(data) {
                        if (data.length) {
                            ...
                            mysql.text_exec(data).then(function (res) {
                                redis.storeCache(data, res).then(function(res){
            ...
            console.log(mData.value, 'closing')
            redis.getCacheKey(`*${mData.value}*`, function(reply){
                if(reply != 'ERRGETKEY') {
                    redis.getCacheValue(reply, function (cacheres) {
                        mysql.text_exec(reply).then(function(result){
                            if(result.length){
                                if(result != cacheres){
    ...
    conn.on('close', function(res) {
        console.log(res, 'someone off line')
    })
})
