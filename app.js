                            ...
                            request(res_opt.value.urlobj, function(error, response, body){
                                if (!error && response.statusCode == 200) {
                                    ...
                                    xmlparser.parseString(body, function (err, result) {
                                        ...
                                        var currentsecs = Math.round(stime/1000)
                                        payprocess.genSecondsSign(payid, currentsecs, function(getsign){
                                            if(getsign)
                                                conn.send(JSON.stringify({key: 'payRequest', value: {timeStamp:currentsecs, nonceStr:getsign.random, package:payid, paySign:getsign.sign}))
                                            else
