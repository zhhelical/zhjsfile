...
wss_server.on('connection', function(conn){
    conn.on('message', function(message) {
        ...
        if(mData.key == 'sessionKey'){
            ...
            payprocess.options(local_order, pay_good, mData.value.price, function(res_opt){
                if(typeof(res_opt.value) != 'string'){
                    request(res_opt.urlobj, function(error, response, body){
                        if (!error && response.statusCode == 200) {
                            //var info = JSON.parse(body)
                            console.log(body)
                            ...
                            xmlparser.parseString(body, function (err, result) {
                                ...
                                conn.send(JSON.stringify({key: 'payRequest', value: result}))
                            })
