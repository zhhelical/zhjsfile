        ...
        return new Promise(function (resolve, reject) {
            mydb.getConnection(function (err, conn) {
                if (err)
                    ...
                    conn.query("SELECT img_name FROM pictures WHERE img_time='" + time + "' AND img_key='" + key_value + "'" + " AND img_pos='" + pos + "'", function (results, err) {
                        conn.release()
                        if (err) reject(err)
                        resolve(results)
                    })
        ...
        return new Promise(function (resolve, reject) {
            mydb.getConnection(function (err, conn) {
                if (err)
                    ...
                    conn.query("SELECT img_name FROM pictures WHERE img_time='" + time + " AND img_pos='" + pos + "'", function (results, err) {
                        conn.release()
                        if (err) reject(err)
                        resolve(results)
                    })
