 ...
 module.exports = {
     ...
     leaveEvent: function(localkey){
         ...
         for(var who in clientsArray){
             if(clientsArray[who].key.localKey === localkey) {
                 ...
                 clientsArray.splice(clientsArray[who], 1)
                 var tol_keys = clientsArray[who].key
                 if(tol_keys.hasOwnProperty('expires_in')){
                     delete tol_keys.expires_in
                     delete tol_keys.session_key
                 }
                 redis.deleteData(JSON.stringify(tol_keys)).then(function (d_res) {}).then(function(err){
                     if(err)
     ...
     recordsExpired: function(recorder){
         ...
         redis.getCacheValue(recorder).then(function (val) {
             ...
             //cb({user:recorder, vals:cache_dels}) add here in future
         }).then(function(err){
             if(err)
