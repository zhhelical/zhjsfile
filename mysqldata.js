...
module.exports = {
            ...
            mydb.getConnection(function(err,conn){
                if(err)
                    ...
                    if(key_value != '') {
                        ...
                        conn.query("SELECT img_pos FROM pictures WHERE img_time='" + time + "'", function (err, results, feilds) {
                            conn.release()
                            if (err) reject(err)
                            if (results.length) resolve(results)
                            else resolve([])
    ...
    select_picname: function(time, key_value, pos) {
        return new Promise(function (resolve, reject){
            mydb.getConnection(function(err,conn){
                if(err)
                    reject(err,null,null)
                else{
                    conn.query("SELECT img_name FROM pictures WHERE img_time='" + time + "' AND img_key='" + key_value + "'" + " AND img_pos='" + pos + "'", function (err, results, feilds) {
                        conn.release()
                        if (err) reject(err)
                        if (results.length) resolve(results)
                        else resolve([])
