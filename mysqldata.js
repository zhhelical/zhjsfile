        ...
        mydb.getConnection(function(err,conn){
            if(err) {
                ...
                conn.query("SELECT * FROM loggers WHERE openid='" + openid + "'", function (err, results, feilds) {
                    ...
                    if (results.length) {
                        var s_res = JSON.parse(results)
                        var time = s_res.time.push(new Date().getTime())
                        var times = s_res[0].times + 1
                        conn.query("UPDATE loggers SET times='" + times + "', time='" + JSON.stringify(time) + "' WHERE openid='" + openid + "'", function (err) {
                            if (err) {
                                var err_obj = {
                                    err_reason: err,
                                    err_table: 'loggers',
                        ...
                        var times = 1
                        conn.query("INSERT INTO payment (openid, times, time) VALUES(" + "'" + openid + "'" + ", " + "'" + times + "'" + ", " + "'" + JSON.stringify(time) + "')", function (err) {
                            if (err) {
                                var err_obj = {
                                    err_reason: err,
                                    err_table: 'loggers',
        ...
        return new Promise(function (resolve, reject){
            mydb.getConnection(function(err,conn){
                if(err)
                    ...
                    conn.query("INSERT INTO errlog (err_reason, err_table, openid, err_time, old_content, new_content, opt_type) VALUES(" + "'" + err_obj.err_reason + "'" + ", " + "'" + err_obj.err_table + "'" + ", " + "'" + err_obj.openid + "'" + ", " + "'" + err_obj.err_time + "'" + ", " + "'" + err_obj.old_content + "'" + ", " + "'" + err_obj.new_content + "'" + ", " + "'" + err_obj.opt_type + "'" + ")", function (err, results){
                        conn.release()
                        if (err) reject(err)
                        if(results.length) resolve(results)
                        else resolve([])
