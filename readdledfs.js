    ...
    , analyseTcIndexPage = function(rdfile, info, cb){
        ...
        if(!dds) {
            ...
            if(!ddsi){
                ...
                ddsi = dds[0].match(revar)
            }
            for (var ii in ddsi) {
                var link = ddsi[ii].match(/<a\s+href=('|")http:\/\/[^\s]+('|")/g)
                if (link) {
                    i_link.push(link[0].replace(/'|"|<a\s+href=/g, ''))
                    var name = ddsi[ii].match(/<a(.|\r|\n)*?(class='u'|title|class="t"\s+rel="nofollow")(.|\r|\n)*?\/a>/g)
                    if (name)
                        names.push(name[0].replace(/<(.*?)>|\r|\n/g, ''))
                    else {
                        name = ddsi[ii].match(/<p\s+class="seller">(.|\r|\n)*?\/p>/g)
                        if(name)
                            names.push(name[0].replace(/<(.*?)>|\r|\n|\t/g, ''))
                        else
                            names.push('null')
                    }
                    var addr = ddsi[ii].match(/(<dd|<span\s+class="fl">)(.|\n|\r)*?(<\/dd>|<\/span>)/g)
                    if (addr) {
                        if(addr.length > 2)
                            addrs.push(addr[addr.length - 2].replace(/<(.*?)>/g, ''))
                        else
                            addrs.push(addr[0].replace(/<(.*?)>|\n|\r|\t/g, '').replace(/\s+/g, ' '))
                    }
                    else
                        addrs.push('null')
                }
...
module.exports = {
    genWebLinks: function(w_site){
        ...
        if(w_site == 'bx')
            ...
            dest_site = tc58Dir
        return new Promise(function (resolve, reject) {
            readDirFiles(dest_site).then(function () {
                resolve('finished gen all links')
            }).then(function (err) {
                if (err) {
