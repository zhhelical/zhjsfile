...
Rectangle {
    ...
    ListModel {
        ...
        ListElement {
            title: "编辑信息"
            collapsed: true
            picture: "/home/dapang/workstation/spc-tablet/images/dinfo.png"
            nested: [ListElement {
                    name: "选取信息"
                    grayed: true
    ...
    Component {
       ...
       Row {
            Rectangle{
                ...
                border.color: "#C4C4C4"
                gradient: Gradient {
                    GradientStop { position: 0.0; color: {if(title==="冻结滑动"||title==="清空选择"||title==="编辑信息"||title==="字体大小"||title==="文字编辑"||title==="表格显示"||title==="单元尺寸"||title==="单元合并"||title==="插删行列"||title==="表格框"||title==="文字颜色"||title==="表背景色"||title==="pdf导出")
                                                        collapsed===false?"#AEEEEE":"#4A708B";
                                                    else
                                                        top_ma.pressed?"#AEEEEE":"#4A708B"
                        }
                    }
                   GradientStop { position: 0.66; color: {if(title==="冻结滑动"||title==="清空选择"||title==="编辑信息"||title==="字体大小"||title==="文字编辑"||title==="表格显示"||title==="单元尺寸"||title==="单元合并"||title==="插删行列"||title==="表格框"||title==="文字颜色"||title==="表背景色"||title==="pdf导出")
                               collapsed===false?"#AEEEEE":"#292929";
                           else
                               top_ma.pressed?"#AEEEEE":"#292929"
                        }
                   }
                   GradientStop { position: 1.0; color: {if(title==="冻结滑动"||title==="清空选择"||title==="编辑信息"||title==="字体大小"||title==="文字编辑"||title==="表格显示"||title==="单元尺寸"||title==="单元合并"||title==="插删行列"||title==="表格框"||title==="文字颜色"||title==="表背景色"||title==="pdf导出")
                ...
                onClicked: {nested_model.setProperty(index, "collapsed", !collapsed);                   
                    if (title==="撤销" || title==="重做" || title==="存储编辑"|| title==="删除存储")
                        sendUndoRedo(title)
                    if (title==="冻结滑动")
