...
var webhead = 'http://'
    ...
    , cleanCommHtml = function(cons){
       // var t_clean = cons.replace(/<head>.*<\/head>/g, '').replace(/<script.*?>.*<\/script.*?>/g, '').replace(/<header>.*<\/header>/g, '')
        var revar = new RegExp("<div class='container box'>(.|\r|\n)*<\/div>", "g")
        var container = t_clean.match(revar)[0]
        container.replace(/<div class='sidebar'>(.|\r|\n)*<\/div>/g, '')
        return container
    }
    ...
    , findSpecialHtml = function(cons, specs){
        var revar = new RegExp(specs, "g")
        for(var di in cons){
            if(cons[di].match(revar))
                return true
        }
        return null
    }
    , labelFindParent = function(cons, strs){
        var nets_cell = <.*?>strs<.*?>, revar = new RegExp(nets_cell, "g"), any_con = '(.|\r|\n)*?'
        var recurs_match = function(m_cons, m_strs){
            revar = new RegExp(m_strs, "g")
            var mp_ps = m_cons.match(revar)
            if(!mp_ps)
                return m_strs
            var m_bodys = '<.*?>'+any_con+m_strs+any_con+'<.*?>'
            recurs_match(m_cons, m_bodys)
        }
        recurs_match(cons, nets_cell)
    }
                    ...
                    if(info.value.match(/>薪资待遇：</g)) {
                        ...
                        for(var ti in head_tabs){
                            if(head_tabs[ti].match(/pos_salary/g))
                                arri_obj.content.add_value[1].contents += head_tabs[ti].match(/pos_salary(.*?)\/span>/g)
            ...
            var recurFetch = function(r_list) {
                if(!r_list.length) {
                    ...
                    var recur_indexes = function(w_list){
                        ...
                        var f_info = {latitude:location.latitude, longitude:location.longitude, lscope:location.old_scope, scope:location.scope, value:w_list[0].value}
                        if(gate=='tapGates' || gate=='tapServes')
