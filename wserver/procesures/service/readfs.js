...
var randomfunc = function(required){
    ...
    , interval_time = function(){
        return 120000+randomfunc(3)*60000
    }
    ...
    , analyseAddrCity = function(addr_obj, gates){
        var fr_city = gates.city.split('市')[0]
        if(fr_city == gates.city)
            fr_city = gates.city.split('省')[0]
        if(!addr_obj.address.match(fr_city))
            addr_obj.address = gates.city+addr_obj.address
        else{
            if(!addr_obj.address.match(gates.city))
                addr_obj.address = gates.city+addr_obj.address.split(fr_city)[1]
        }
    }
    , analyseWebForGate = function(gates, info, cb){
        ...
        for(var ci in cnStrs) {
            if(gates.type.match('bx')) {
                ...
                var addr_test = cnStrs[ci].match(/<li>(.|\n|\r)*?<\/li>/g)
                if(addr_test)
                    addr = addr_test[0].replace(/<.*?>|&nbsp;/g, '')
            }
        }
        ...
        reanynisAddr(arri_obj.address).then(function (a_res) {
            ...
            for(var ci in cnStrs) {
                if(gates.type.match('bx')) {
                    ...
                    if(detail){
                        var artical = detail.match(/<article(.*?)<\/article>/g)
                        if(artical)
                            arri_obj.content.add_value[2].contents = artical[0].replace(/<.*?>/g, '\n').replace(/\n+/g, '\n')
                    }
                }
            }
            if(gates.type.match('bx')) {
    ...
    , analyseWebForJob = function(gates, info, cb){
        ...
        for(var ci in cnStrs) {
            if(gates.type.match('bx')) {
                ...
                if(test_addr)
                    addr = test_addr.replace(/<.*?>/g, '').replace(/.*?>/g, '')
            }
        }
        ...
        if(!arri_obj.address) {
            ...
            analyseAddrCity(arri_obj, gates)
        var chkAddr = function(addr_obj, callback){
            reanynisAddr(addr_obj.address).then(function (a_res) {
                if(a_res == '系统错误，请联系管理员！')
                    callback('fail')
                else {
                    analyseAddrCity(addr_obj, gates)
                    callback(a_res)
                }
            }).then(function(err){
                if (err)
        ...
        var analyseJob = function(t_obj, geo_res){
            t_obj.location.longitude = geo_res.lng
            t_obj.location.latitude = geo_res.lat
            var p_labels = labelFindParent(info, '首次发布于')
            if(gates.type.match('58')) {
                ...
                if(p_labels && p_labels.match(/今天/g))
                    ...
                    p_labels = time_gen.formatMinuteTime(c_date.setTime(old_time))
                }
            }
            t_obj.time = p_labels.replace(/<.*?>|首次发布于：|更新：|&nbsp;/g, '').replace(/.*?>/g, '')
            ...
            for(var ci in cnItStrs) {
                if(gates.type.match('bx')) {
                    if (cnItStrs[ci].match(/职位类别：/g)) {
                        ...
                        t_obj.content.add_value[2].contents = j_type.replace(/<.*?>|类别|&nbsp;/g, '').replace(/.*?>/g, '')
                    }
                    if (cnItStrs[ci].match(/招聘人数：/g)) {
                        var j_pers = labelFindParent(cnItStrs[ci], '招聘人数：<\/')
                        t_obj.content.add_value[2].contents += '\n' + j_pers.replace(/<.*?>|&nbsp;/g, '').replace(/.*?>/g, '')
                    }
                    ...
                    t_obj.content.add_value[2].contents = j_type.replace(/<.*?>/g, '')
            ...
            if(c_phone)
                t_obj.content.add_value[3].contents = '联系电话：'+c_phone[0].replace(/<.*?>/g, "").replace(/>/g, '')
            else
                t_obj.content.add_value[3].contents = '对不起，对方未留下联系方式'
            ...
            if(cndprStrs) {
                if(gates.type.match('bx'))
                    t_obj.content.add_value[1].contents = cndprStrs[0].replace(/<br \/>/g, '\n').replace(/<.*?>|职位描述|&zwnj;|[^\u4e00-\u9fa5|\u3002|\uff1f|\uff01|\uff0c|\u3001|\uff1b|\uff1a|\u201c|\u201d|\u2018|\u2019|\uff08|\uff09|\u300a|\u300b|\u3008|\u3009|\u3010|\u3011|\u300e|\u300f|\u300c|\u300d|\ufe43|\ufe44|\u3014|\u3015|\u2026|\u2014|\uff5e|\ufe4f|\uffe5|0-9|-]*/g, '')
                else
                    t_obj.content.add_value[1].contents = cndprStrs.replace(/<(.*?)>/g, '')
            }
            cb(t_obj)
        }
        chkAddr(arri_obj, function(c_res){
            if(c_res != 'fail'){
                if(addr == ''){
                    reanynisAddr(arri_obj.address).then(function (s_res) {
                        if(s_res == '系统错误，请联系管理员！')
                            return cb(null)
                        analyseJob(arri_obj, s_res)
                    }).then(function(err){
                        if (err)
                            return cb(null)
                    })
    ...
    , analyseWebForServe = function(gates, info, cb){
        ...
        if(gates.type.match('58')) {
            cnStrs = info.match(/<div\s+class="newinfo">(.|\n|\r)*?详细地址(.|\n|\r)*?<\/div>/g)
            var area_test = info.match(/<div\s+class="newinfo\s+serviceInfo\s+">(.|\n|\r)*?<\/li>/g)
            if(area_test)
        ...
        reanynisAddr(arri_obj.address).then(function (a_res) {
            ...
            revar = new RegExp("<div class='viewad-text'>(.|\r|\n)*?\/div>", "g")
            if(gates.type.match('58'))
                revar = new RegExp("<div\\s+class=\"descriptionBox\">(.|\r|\n)*?<\/div>", "g")
            ...
            if(view_add) {
                if(gates.type.match('bx'))
                    ...
                    var test_desc = view_add[0].match(/<article\s+class="description_con">(.|\r|\n)*?<\/article>/g)
                    if(test_desc) {
                        var t_absort = test_desc[0].match(/<p>(.*?)<\/p>/g)
    ...
    , analyseWebForRend = function(gates, info, cb){
        ...
        if(gates.type.match('58')) {
            ...
            console.log(arri_obj.address, 'clear?')
        }
        else
            arri_obj.address = p_labels.replace(/<.*?>|地址：|&nbsp;/g, '')//.replace(/.*?>/g, '')
        if(arri_obj.address == '')
            return cb(null)
        analyseAddrCity(arri_obj, gates)
        reanynisAddr(arri_obj.address).then(function (a_res) {
            ...
            arri_obj.content.add_value[1].name = p_labels.replace(/<.*?>|&nbsp;/g, '').replace(/.*?>/g, '')
            if(gates.type.match('bx')) {
                if (gates.noseq == 0) {
                ...
                for(var pli in p_labels)
                    arri_obj.content.add_value[2].contents += p_labels.replace(/<.*?>/g, '')+'\n'
                p_labels = findSpecialHtml(info, '<div\\s+class=\"house-word-introduce(.|\r|\n)*?<\/div>').match(/<b>(.*?)<\/b>/g)
                for(var pli in p_labels)
                    arri_obj.content.add_value[3].contents += p_labels.replace(/<\/b>/g, '\n').replace(/<.*?>/g, '')
            }
            revar = new RegExp("<strong>[0-9|-]*<\/strong>", "g")
            var p_num = info.match(revar)
            if(gates.type.match('58'))
            ...
            if(p_num)
                arri_obj.content.add_value[4].contents = p_num[0].replace(/<.*?>/g, "")
            revar = new RegExp("<div\\s+class='photo-item(.*?)<\/div>", "g")
            if(gates.type.match('58'))
    ...
    , analyseWebForOld = function(gates, info, cb){
        ...
        arri_obj.address = p_labels.replace(/.*<span/g, '').replace(/<.*?>|&nbsp;/g, '').replace(/.*?>/g, '')
        if(arri_obj.address == '')
            return cb(null)
        analyseAddrCity(arri_obj, gates)
        reanynisAddr(arri_obj.address).then(function (a_res) {
            ...
            revar = new RegExp("<strong>[0-9|-]+<\/strong>", "g")
            if(gates.type.match('58'))
                revar = new RegExp("<span\\s+id=\"t_phone\"(.*?)<\/span>", "g")
            var p_num = info.match(revar)
            if(p_num)
                arri_obj.content.add_value[3].contents = p_num[0].replace(/<.*?>/g, "")
            ...
            var rd_files = function(rdfs, rdcitys){
                ...
                if(rdfs[0].match(/links/g)) {
                    ...
                    file.readFile(f_path, function (err, res) {
                        ...
                        f_obj.city = reanynisCity(rdcitys[0], destination)
                        var caches = {}
